...
public final class AciSubnetAnalyzer {
   ...
   */
  @Nonnull
  public static List<SubnetFinding> analyzeSubnets(AciConfiguration config) {
  ...
  private static List<SubnetFinding> checkOverlappingSubnets(List<BridgeDomainInfo> bdInfoList) {
    ...
    for (BridgeDomainInfo bdInfo : bdInfoList) {
      String vrfKey = bdInfo.vrf != null ? bdInfo.vrf : "NO_VRF";
      for (String subnetStr : bdInfo.subnets) {
        SubnetInfo subnetInfo =
            new SubnetInfo(bdInfo.tenant, bdInfo.bdName, bdInfo.fqBdName, vrfKey, subnetStr);
    ...
    for (Entry<String, List<SubnetInfo>> vrfEntry : subnetsByVrf.entrySet()) {
      ...
      for (int i = 0; i < vrfSubnets.size(); i++) {
        for (int j = i + 1; j < vrfSubnets.size(); j++) {
          ...
          if (subnet1.fqBdName.equals(subnet2.fqBdName)) {
            continue;
          }

          // Skip identical subnets (handled by duplicate check)
          if (subnet1.subnet.equals(subnet2.subnet)) {
            continue;
          }

          if (subnetsOverlap(subnet1.subnet, subnet2.subnet)) {
            findings.add(
                new SubnetFinding(
    ...
    for (int i = 0; i < allSubnets.size(); i++) {
      for (int j = i + 1; j < allSubnets.size(); j++) {
        ...
        if (subnet1.vrf.equals(subnet2.vrf)) {
          continue;
        }
                  ...
                  new SubnetFinding(
                      ...
                      String.format(
                          "Duplicate subnet '%s' configured in multiple bridge domains: '%s' and"
                              + " '%s'",
                          entry.getKey(), subnet1.bdName, subnet2.bdName),
                ...
                new SubnetFinding(
                    ...
                    String.format(
                        "Invalid subnet '%s' in bridge domain '%s': prefix length %d exceeds"
                            + " maximum of 32",
                        subnetStr, bdInfo.bdName, prefixLength),
          ...
          findings.add(
              new SubnetFinding(
                  ...
                  String.format(
                      ...
                      subnetStr, bdInfo.bdName, e.getMessage()),
  ...
  private static class BridgeDomainInfo {
    final String tenant;
    final String bdName;
    final String fqBdName;
    final @Nullable String vrf;
    ...
    BridgeDomainInfo(String tenant, String bdName, AciConfiguration.BridgeDomain bd) {
      ...
      this.bdName = bdName.substring(bdName.indexOf(':') + 1);
      this.fqBdName = bdName;
      this.vrf = bd.getVrf();
      this.subnets = bd.getSubnets() != null ? bd.getSubnets() : ImmutableList.of();
    }
  }
